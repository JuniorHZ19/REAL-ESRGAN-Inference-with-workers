...
set /p input="Ingrese nombre imagen de entrada ,que esta en la ruta inputs/video:"
...
set /p num_process="Ingrese numero de procesos a la vez:"

set /p subfijo="Ingrese el subfijo que se ponndra al output:"

set /p tile="Ingrese el size tile:"

python inference_realesrgan_video.py -i inputs/video/%input% -n %model_name% -s %scale% --suffix %subfijo% --tile %tile% --num_process_per_gpu %num_process%
