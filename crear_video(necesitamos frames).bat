...
ffmpeg -framerate %framerate% -i  "%BASE_DIR%inputs\frames\frame_%%04d.png" -i %BASE_DIR%inputs\video\%input% -map 0:v -map 1:a -c:v libx264  -c:a mp3 -crf 15 -preset veryslow -r %rate%  results/%ouput_sf%.mp4
